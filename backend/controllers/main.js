...
const mainflow = async (req, res) => {
  try {
    ...
    if (body?.isMessage) {
      ...
      if (
        typeOfMsg === "location_message" &&
        incomingMessage.location.hasOwnProperty("latitude")
      ) {
        ...
        const { data: user, error: err } = await supabase
          .from("main_table")
          .update({
            user_lat: incomingMessage.location.latitude,
            user_lon: incomingMessage.location.longitude,
          })
      ...
      if (incomingMessage.text && incomingMessage.text.hasOwnProperty("body")) {
        ...
        if (user[0].user_flag === true) {
          const processtext = JSON.parse(
            await process_text(incomingMessage.text.body)
          );
          if (user[0].language === "malayalam") {
            console.log("malayalam");
            const message = {
          ...
          .insert([
            {
              ...
              image_link:
                "https://wknqtqxfmrqcgvihsulz.supabase.co/storage/v1/object/public/images/jeff.jpeg",
            },
          ]);
